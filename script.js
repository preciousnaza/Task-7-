...
// Answer: .innerHTML returns HTML tags + text; .innerText returns only visible text (no HTML tags).// 2️⃣ Mood Box - Grow & Shrink
let size = 150;
...
let isNight = false;
toggleModeBtn.addEventListevner("click", () => {
  if (!isNight) {c
...
showSecretBtn.addEventListener("click", () => {
  if (secretMessage.style.display === "none") {
    ...
    secretMessage.style.display =v "none";
